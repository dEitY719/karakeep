...
- 모드 스위치: `~/.dotfiles-setup-mode` 파일에 `internal` | `external` | `home` 중 하나.
  dotfiles 가 공용 PC 를 `public` 으로 명명하므로 그 값도 그대로 허용하며, karakeep
  파이프라인은 이를 `home` 의 별칭으로 처리한다 (별도 public 동작 없음 = `home` 과 동일).
  코드상으로도 `is_work` 만 갈리므로(`mode == "internal"`), `public`/`home`/`external`
  은 모두 `is_work=False` 로 동일 취급된다.
...
2. `~/.dotfiles-setup-mode` 읽어 모드 결정 (internal/external/home; `public` 은 `home` 별칭).
