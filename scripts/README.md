...
7. internal 전용 — `80-Company/Bookmarks` 를 GHES 에서 clone/pull (read/write).
...
# internal 모드에서만:
BOOKMARKS_COMPANY_REMOTE="https://<user>:<ghes-pat>@<ghes-host>/<org>/bookmarks-company.git"
...
- 회사 노트(`80-Company`)는 obsidian-para 에서 `.gitignore` 로 제외되어 GitHub 로 새지
  않는다. 그 안의 `Bookmarks/` 만 GHES repo 로 별도 관리된다.
- 전체 vault 노트의 PC 간 공유 전략(Syncthing 등)은
  [docs/pc-environment.md](../docs/pc-environment.md) §6 참조.
